...
```bash
~> w3m -cols 1024 -dump https://www.rfc-editor.org/rfc-index.html | awk -F'[\\[|\\]]' '/^[0-9][0-9][0-9][0-9]/ && !/Not Issued/{print $2}' | xargs -I {} env TZ=Europe/London date -d'01{}' +"%Y" | sort -n | uniq -c | gn
uplot -e "set term dumb size 145, 25; set xtics 3; plot '-' with lines notitle"
line 56: warning: Too many axis ticks requested (>2e+02)
...
```bash
~> curl -s https://mirror.funkfreundelandshut.de/ietf/internet-drafts/all_id.txt | awk '/^draft/{print $2}' | xargs -I {} env TZ=Europe/London date -d'{}' +"%Y-%m" | sort -n | uniq -c | awk '{print $2" "$1}' | gnuplot-e "set term dumb size 145, 25; set xtics 2; set ytics 20; plot '-' notitle smooth csplines"
